...
-- GRP_NO 는 원글과 원글에 딸린 댓글을 구별하기 위한 값으로
-- 원글만 존재할 경우에는 GRP_NO 는 NULL 로 주고
-- 만약, 댓글이 달리면 그때 원글에 RCD_NO와 동일한 값을 부여하는 것으로 한다.
-- 이때, 댓글도 원글과 GRP_NO 는 동일하게 묶는다.
-- RCD_LEVEL 은 이 글이 원글인지 댓글인지 대댓글인지 나타내는 값으로 
-- 원글이면 0, 1단계댓글이면 1, 2단계 대댓글이면 2 이런식으로 추가 된다.
-- RCD_ORDER 은 댓글이든, 원글이든, 대댓글이든 시간순서대로 보여주는 값
