...
$(function() {
	$.getJSON(context+'/admin/member_list/1',function(data) {
		...
		+'<TR><TD ALIGN=LEFT WIDTH=100><IMG SRC="${img}/btn_new.gif"'
		+'onClick="" STYLE="CURSOR:HAND"></TD>'
		+'<TD WIDTH=320 ALIGN=CENTER><c:if test="${startPage ne 1}">'
		+'<a href="${context}/event/boardList/1"><IMG SRC="${img}/btn_bf_page.gif">&nbsp;</a> '
		+'</c:if><c:if test="${startPage -GROUPSIZE gt 0}">'
		...
		+'<c:choose><c:when test="${status.index == pageNo}">'
		+'<font style="color:red; font-size: 20px">${status.index}</font></c:when>'
		+'<c:otherwise>'
		+'<a href="${context}/event/boardList/${status.index}">'
		+'<font>${status.index}</font></a></c:otherwise></c:choose>'
		+'</c:forEach><c:if test="${startPage + GROUPSIZE le totPage}">'
		+'a href="${context}/event/boardList/${startPage+GROUPSIZE}">'
		...
		+'<input type="submit" value="검 색" /></FORM></TD></TR></TABLE>';
		$('.mainView').html(table);
	});
